...
export default function CaseEditorPage() {
  ...
  const caseId = params?.id ? (params.id as string) : 'standalone';
  ...
  /* ── Load STLs from Backend ──────────────────────────── */

  const loadCaseSTLs = useCallback(async () => {
    setLoading(true);
    try {
      if (caseId && caseId !== 'standalone') {
        const res = await fetch(`${API_BASE}/api/cases/${caseId}/stls`);
        if (res.ok) {
          const data = await res.json();
          const allStls = data.stls || [];

          const latestByLayer = new Map<string, typeof allStls[0]>();
          for (const item of allStls) {
            const existing = latestByLayer.get(item.layer_id);
            if (!existing || new Date(item.created_at) > new Date(existing.created_at)) {
              latestByLayer.set(item.layer_id, item);
            }
          }
          const stls = Array.from(latestByLayer.values());
          ...
          for (const item of stls) {
            try {
              const rawPath = item.download_url || item.file_path || `/api/cases/${caseId}/stls/${item.id}/download`;
              const downloadUrl = rawPath.startsWith('http')
                ? rawPath
                : `${API_BASE}${rawPath.startsWith('/') ? '' : '/'}${rawPath}`;

              const fileRes = await fetch(downloadUrl, { cache: 'no-cache' });
              ...
              console.error('Failed to load STL file', item.file_name || item.filename || item.id, err);
            }
          }
        }
      }
    } catch (e) {
      console.error('Error fetching STLs', e);
    } finally {
      setLoading(false);
    }
  }, [caseId, clearObjects, addObject]);
  ...
  const handleLocalSTLUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        addObject(newObj);
        selectObject(id);
        success('STL Imported', `${file.name} loaded into 3D Editor`);
      } catch (err) {
  ...
  const fetchMeshStats = useCallback((objId: string) => {
    const obj = useEditorStore.getState().objects.get(objId);
    if (!obj) {
      setMeshStats(null);
      return;
    }
  ...
  useEffect(() => {
    if (activeObj) {
      fetchMeshStats(activeObj.id);
    } else {
      setMeshStats(null);
    }
  }, [activeObj?.id, fetchMeshStats]);
  ...
  const handleSplitDisconnected = async () => {
    ...
    try {
      ...
      success(
        ...
        `Extracted ${components.length} discrete anatomical components (zero positional drift)`
      );
    } catch (e) {
  ...
  /* ── Mesh Tools (Smooth, Repair, Decimate) ───────────── */

  const handleSmoothMesh = async () => {
    if (!activeObj) return;
    setMeshToolProcessing(true);
    try {
      const smoothedGeo = autoHealMeshClient(activeObj.geometry);
      updateObject(activeObj.id, { geometry: smoothedGeo });
  ...
  const handleRepairMesh = async () => {
    ...
    setMeshToolProcessing(true);
    try {
      const repairedGeo = autoHealMeshClient(activeObj.geometry);
      updateObject(activeObj.id, { geometry: repairedGeo });
  ...
  const handleDecimateMesh = async () => {
    if (!activeObj) return;
    setMeshToolProcessing(true);
    try {
      const simplifiedGeo = autoHealMeshClient(activeObj.geometry);
      updateObject(activeObj.id, { geometry: simplifiedGeo });
  ...
  const runNetfabbDiagnostics = useCallback(async () => {
    if (!activeObj) return;
